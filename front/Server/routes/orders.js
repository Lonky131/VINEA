...
router.get('/:id', (req, res) => {
  ...
  .then(orders => {
    if (orders.length > 0) {
      ...
      res.json({message: `No orders found by this id: ${orderId}`})
    }
  }).catch(err => {console.error(err)});
    ...
    database.table('orders')
      .insert({
        ...
        if (newOrderId.insertId > 0){
          ...
          res.json({message: 'new order failed while adding order details', success: false, body: JSON.stringify(newOrderId.insertId)})
        }
